...
function levelslider(hObj,event,lineseries_handles,Img)
...
start=1;
finish=sum(cell_idx==cellnum);
%com=com_full(cell_idx==cellnum,:);
%L3=sum(cell_idx==cellnum);
L3=finish-start+1;
Cells_cell=NewCellArray(cell_idx==cellnum);
temp=cell2mat(Cells_cell(start:finish));
xmin=min(temp(:,1));
xmax=max(temp(:,1));
ymin=min(temp(:,2));
ymax=max(temp(:,2));
...
clear(sprintf('ImageStack%03d',stacknum))
Img=zeros(ymax-ymin+1,xmax-xmin+1,size(Img2,3));
for i=1:size(Img2,3)
    Img(:,:,i)=flipud(Img2(xmin:xmax,ymin:ymax,i)');
end
clear('Img2')
for i=1:size(Img,3)
    M=max(max(Img(:,:,i)));
...
uicontrol('Style', 'slider',...
    ...
    'Callback', {@levelslider,plandles,Img});
