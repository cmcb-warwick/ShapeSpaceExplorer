...
function CellSegmentation(files, framesConfig, msConfig, savefolderpath)
...
%   msConfig.minAreaSize = minimum area a cell must have to be added.
%
...
for i=1:N;
   ...
   currentFrame=StackCellSeg(in.stack, i, savefolderpath, msConfig.spatialBdw,msConfig.rangeBdw, msConfig.minArea, currentFrame, allFrameNum, h);
end
delete(h)
close all force
...
writeGroups2Table(tableFilename,originFileName,stackFileName)
...
function writeGroups2Table(path,originFileName,stackFileName)
header='OriginalName,  ConvertedName';
dlmwrite(path,header,'delimiter','');
s=size(originFileName);
for i=1:s(1)
    txt = [originFileName{i,1} ',' stackFileName{i,1}];
...
function cFrame = StackCellSeg( ImageStack,Stacknumber,savefolderpath,sbw,rbw,minArea, currentFrame, allFramNum, h)
%STACKCELLSEG Summary of this function goes here
%   Detailed explanation goes here
numframes=size(ImageStack,3);
mF=mean(ImageStack(:));
cFrame=currentFrame;
anustack=zeros(size(ImageStack));
for i=1:numframes
    frame=ImageStack(:,:,i);
...
for i=1:numframes
   [Frame_curves{i},Bin_images{i}] =CellSeg(anustack(:,:,i),sbw,rbw, minArea);
...
function [ Image_Cell_Regions,Binary_images ] = CellSeg( GRAYimage,sbw,rbw, minArea)
...
for i=regionlist(InteriorRegion)'
    ...
    border =bound{1};
    polyArea=polyarea( border(:,2), border(:,1));
    if (polyArea<minArea), continue; end % if too small don't add area.
    Image_Cell_Regions{end+1}=border;
